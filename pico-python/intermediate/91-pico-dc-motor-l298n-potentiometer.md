# 91 - Pico DC Motor L298N Potentiometer

Build a DC motor speed console that displays the current motor speed percentage and rotation direction on the serial monitor.

## Goal
Learn how to read an analog potentiometer value (0–65535), calculate motor speed percentages, control H-bridge driver direction pins, and output status logs in MicroPython.

## What You Will Build
A DC motor control dashboard:
...
- **Serial Output**: Prints the active direction and speed percentage to the console.
...
print("DC motor driver dashboard active.")
...
while True:
    ...
    if raw > 34000:
        ...
        print("Direction: FORWARD | Speed:", percent, "%")
    elif raw < 31000:
        # Scale speed: maps raw range to PWM duty (0 to 65535)
        diff = 31000 - raw
        ...
        percent = duty * 100 // 65535
        print("Direction: REVERSE | Speed:", percent, "%")
    else:
        # Stop motor
        in1.value(0)
        in2.value(0)
        ena.duty_u16(0)
        print("Direction: STOPPED | Speed: 0 %")
        
    utime.sleep_ms(250) # Status log rate
```
...
4. Slide the potentiometer slider left and right to control the DC motor and check the terminal logs.

## Expected Output
```
DC motor driver dashboard active.
Direction: STOPPED | Speed: 0 %
Direction: FORWARD | Speed: 45 %
Direction: REVERSE | Speed: 62 %
```
