# 76 - Pico Magnetic Security Alarm

Build a door-entry security alarm that sounds a warning buzzer when a magnetic switch (Hall sensor) detects that a door has been opened (magnet removed).

## Goal
Learn how to read digital inputs from Hall effect sensors, handle active-low logic levels, and implement latching entry alarms in MicroPython.

## What You Will Build
A magnetic door alarm:
...
> **Wiring tip:** The Hall sensor output connects to GP14, and the reset button connects to GP13 (both using internal pull-ups). Connect the active buzzer VCC to GP15. All ground pins are shared.
...
print("Magnetic security system armed.")
...
1. Drag the **Raspberry Pi Pico**, **two Push Buttons** (representing door sensor and reset), and **Active Buzzer** onto the canvas.
2. Connect Door Sensor Button to **GP14**, Reset Button to **GP13**, and Buzzer to **GP15**. Connect all grounds.
...
- Clicking the Door Sensor button component causes the buzzer component to pulse active.
- Clicking the Reset Button stops the buzzer.
...
- [50 - Pico Hall Magnetic Sensor Serial](50-pico-hall-magnetic-sensor-serial.md)
