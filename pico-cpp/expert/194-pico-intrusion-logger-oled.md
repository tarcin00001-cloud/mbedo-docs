# 194 - Pico Intrusion Logger OLED

Build an advanced dual-beam laser security grid that draws real-time LDR beam strength graphs on an OLED, latches a siren relay, and logs breaches over Bluetooth.

## Goal
Learn how to control light transmitters (lasers), read analog LDR receivers in parallel, latch alarm relays, design graphic OLED status bars, and stream Bluetooth audit warnings.

## What You Will Build
A wireless security grid gateway with graphical signal HUD:
...
- **SSD1306 OLED (GP4, GP5)**: Draws two horizontal signal strength bars side by side, and logs status alerts.
...
void loop() {
  ...
  // Map LDR values (0-4095) to horizontal bar widths (0-60 pixels)
  int barWidth1 = ldr1 * 60 / 4095;
  int barWidth2 = ldr2 * 60 / 4095;
  ...
  if (alarmLatched) {
    // Show alarm status overlay at the bottom
    display.setCursor(10, 24);
    display.print("SYSTEM BREACHED!");
    display.setCursor(10, 40);
    display.print(breachMessage);
  } else {
    // Show active signal strength thermometer bars
    display.setCursor(6, 20);
    display.print("B1:");
    display.drawRect(35, 18, 64, 12, SSD1306_WHITE);
    if (barWidth1 > 0) {
      display.fillRect(37, 20, barWidth1, 8, SSD1306_WHITE);
    }

    display.setCursor(6, 36);
    display.print("B2:");
    display.drawRect(35, 34, 64, 12, SSD1306_WHITE);
    if (barWidth2 > 0) {
      display.fillRect(37, 36, barWidth2, 8, SSD1306_WHITE);
    }

    display.setCursor(6, 52);
    display.print("GRID: SECURE");
  }
...
* Normal state: OLED displays `B1` and `B2` signal strength fill bars. Relay is OFF.
* Beam 1 broken (LDR 1 < TRIP_LIMIT): OLED displays `SYSTEM BREACHED!` / `BEAM 1 CUT`. Relay turns ON (Siren ON) and stays latched. Bluetooth sends alerts.
* Type `'R'` on BT: System resets, OLED returns to signal bar graphs.
...
- [177 - Pico Intrusion Datalogger OLED](177-pico-intrusion-datalogger-oled.md)
