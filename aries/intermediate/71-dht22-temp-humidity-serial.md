# 71 - DHT22 Temp & Humidity Serial Logs
...
## What You Will Build
A DHT22 sensor is connected to GPIO 12 of the ARIES v3 board. The board queries the sensor every 2 seconds, extracts temperature and humidity readings, and prints formatted logs to the Serial Monitor.
...
1. Drag the **VEGA ARIES v3** and **DHT22** components onto the canvas.
...
6. Adjust the temperature and humidity sliders on the DHT22 widget and observe the updated values in the Serial Monitor.
...
* Moving the sliders on the DHT22 component updates the output on the next loop cycle.

## Code Walkthrough
| Line | Check / Action |
| --- | --- |
| `#include <DHT.h>` | Imports the Adafruit DHT Sensor Library. |
| `DHT dht(DHTPIN, DHTTYPE)` | Configures the sensor driver with GPIO 12 and device type. |
...
* **One-Wire Digital Sensor Protocol**: Unlike analog sensors that output a continuous voltage level, the DHT22 uses a custom single-wire digital protocol. The ARIES board triggers a start signal by pulling the data line LOW for 18 ms, then releasing it. The DHT22 responds with a handshake followed by 40 bits of data (16 bits humidity, 16 bits temperature, 8 bits checksum). The library automatically decodes these pulses. Digital transmission prevents signal degradation over long wire lengths compared to analog sensor signals.

## Try This! (Challenges)
1. **Fahrenheit Logging**: Modify the code to print the temperature in Fahrenheit by passing `true` to `dht.readTemperature(true)`.
2. **Min/Max Tracker**: Add state variables to track and print the maximum and minimum temperatures recorded since startup without using arrays.

## Troubleshooting
| Symptom | Likely Cause | Fix |
| --- | --- | --- |
| "Failed to read from DHT22 sensor!" | Missing pull-up resistor or incorrect GPIO pin | Verify DATA is connected to GPIO 12; check if using a module with a built-in pull-up resistor. |
...
| Readings show NaN | Loose connection | Re-seat wires or swap the DHT22 module. |
